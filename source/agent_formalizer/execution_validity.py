...
def _load_events(cell_dir: Path) -> tuple[list[dict], dict]:
    ...
    for line_number, line in enumerate(raw.splitlines(), 1):
        ...
        if event.get("action") not in {"invalidate", "reinstate"}:
            raise ExecutionValidityError(
...
def _identity_from_sources(
    ...
    if len(runtime_hashes) > 1:
        historical_runtime_hashes = set(runtime_hashes)
        if supplied_runtime_hash:
            historical_runtime_hashes.discard(supplied_runtime_hash)
        if not (
            supplied_runtime_hash in authorized_repair_runtime_hashes
            and len(historical_runtime_hashes) == 1
        ):
            raise ExecutionValidityError(
...
def append_manual_event(
    ...
    *,
    ...
    if action not in {"invalidate", "reinstate"}:
        raise ExecutionValidityError("action must be invalidate or reinstate")
    ...
    elif action == "invalidate" and is_tool_infra and any(
