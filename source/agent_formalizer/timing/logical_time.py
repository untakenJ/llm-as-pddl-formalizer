...
def logical_now():
    if os.environ.get("BENCHMARK_EXTERNAL_CALL_TIMING") == "call-checkpoint-v1":
        from python_checkpoints import now
        return now()
...
class Deadline:
    ...
    def __init__(self, seconds):
        ...
        self.local = None
        if os.environ.get("BENCHMARK_EXTERNAL_CALL_TIMING") == "call-checkpoint-v1":
            from python_checkpoints import Deadline as LocalDeadline
            self.local = LocalDeadline(seconds)
            self.target = self.local.target
            return
...
class DeadlineBroker:
                    ...
                    with owner.lock:
                        if msg["op"] == "register":
                            ...
                            result = {"id": key, "target": target,
                                      "physical_monotonic": time.monotonic()}
                        elif msg["op"] == "close":
                            ...
                            result = {"ok": True}
                        elif msg["op"] == "failure" and hasattr(owner, 'failure'):
                            # Explicit adapter protocol failure, never inferred
                            # from an ordinary native timeout or exit status.
                            owner.failure = 'call_checkpoint_control_failed'
                            owner.record({'event': 'native_control_failure', 'pid': pid})
                            result = {"ok": True}
                        else:
    ...
    def settle(self, call_id, seconds):
        with self.lock:
            ...
            if due:
                # Publish the exact due target, not a value one floating-point
                # ulp below it. All native runtimes must see the same verdict.
                self.logical = max(self.logical, *(self.leases[key]['target'] for key in due))
