...
static void rpc(const char *request, char *reply, size_t size) {
    ...
    if (snprintf(addr.sun_path, sizeof addr.sun_path, "%s/broker.sock", directory)
            >= (int)sizeof addr.sun_path) _exit(125);
    ...
    if (fd < 0 || connect(fd, (void *)&addr, sizeof addr) ||
            send(fd, request, strlen(request), MSG_NOSIGNAL) != (ssize_t)strlen(request)) _exit(125);
    size_t pos = 0;
    while (pos + 1 < size) {
    ...
    close(fd);
    reply[pos] = 0;
    if (!pos || strstr(reply, "\"error\"")) _exit(125);
}
...
static void arm(timer_t timer, double seconds, int alarm) {
    ...
    if (seconds > 0) {
        ...
        id = strchr(id, ':') + 1;
        while (*id == ' ') ++id;
        if (*id++ != '"' || strlen(id) < 33 || id[32] != '"') _exit(125);
        ...
        *job = (struct watch){generation, timer, alarm};
        pthread_t thread;
        if (pthread_create(&thread, NULL, watch, job)) _exit(125);
...
__attribute__((constructor)) static void init(void) {
    ...
    if (active) {
        if (strlen(dir) >= sizeof directory) _exit(125);
