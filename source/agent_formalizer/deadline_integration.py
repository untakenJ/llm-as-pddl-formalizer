"""Explicit native runtime/host integration for logical-deadline-v1."""
...
RUNTIME = ROOT / "deadline_runtime"
SUPPORTED_HARNESSES = frozenset({"generic", "nanobot", "hermes", "openclaw"})
ENVIRONMENT_KEYS = ("BENCHMARK_DEADLINE_DIR", "BENCHMARK_DEADLINE_HARNESS", "PYTHONPATH", "LD_PRELOAD")
...
def validate(adapter):
    if selected(adapter):
        supported = {"openclaw"} if policy(adapter) == call_checkpoint.POLICY_ID else SUPPORTED_HARNESSES
...
def prepare(workspace):
    ...
    if checkpoint:
        files.append(ROOT / "call_checkpoint.py")
    if adapter.name == "openclaw":
        from . import openclaw_deadlines
        from .config import OPENCLAW_MODULE_DIR
    ...
    bundle = ROOT.parent.parent / ".cache" / "deadline-runtime" / digest
    ...
    if workspace.artifact_dir:
        write_json(workspace.artifact_dir / "logical_deadline_manifest.json", {
            ...
            "coverage": [name, "GNU timeout POSIX timer", "Python gateway socket reads", "httpcore AnyIO reads"] +
                        (["OpenClaw LLM idle watchdog", "OpenClaw run/foreground exec deadlines",
                          "OpenClaw fetch and OpenAI SDK abort deadlines"] if native_manifest else []),
            "limitations": ["arbitrary user timers", "parallel/background observational equivalence", "post-commit stream recovery"] +
                           (["unlisted provider-specific SDK watchdogs", "local-provider startup guards",
                             "physical timing metadata and cleanup timers"] if native_manifest else []),
        })
