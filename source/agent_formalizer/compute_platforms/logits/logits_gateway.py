...
if __package__:
    from .logits_openai_bridge import DEFAULT_MODEL_ASSETS_ROOT, start_bridge_server
else:  # Direct host script or flat standalone sidecar mount.
    from logits_openai_bridge import DEFAULT_MODEL_ASSETS_ROOT, start_bridge_server
...
def main() -> None:
    ...
    try:
        ...
        if __package__:
            from agent_formalizer.gateways import model_gateway
        else:
            directory = Path(__file__).resolve().parent
            if (directory / "model_gateway.py").is_file():
                # Sidecars mount the shared gateway beside this entrypoint.
                import model_gateway
            else:
                # Host entrypoints live in agent_formalizer/compute_platforms/logits/.
